...
class addNewWord(webapp2.RequestHandler):
            ...
            if self.request.POST.has_key('button'):
                if self.request.POST.has_key('word'):
                    ...
                    if len(anagram_data) == 0:
                        ...
                        self.increase_count(anagram_data, anagram_word)
                    ...
                    for a_word in words:
                        ...
                        if len(word) > 0:
                            ...
                            if len(anagram_data) == 0:
                                ...
                                self.increase_count(anagram_data, word)
            template_values = {
                'user': user,
                'anagram_data': user_anagram_data
            }
    ...
    def add_anagram(self, anagram_word, user_id):
        try:
            ...
            anagram_details = Anagram(word=anagram_word,user_key = sorted_key,sorted_word=sorted_word, user_id=user_id, word_count=1,letter_count=len(sorted_word))
            anagram_details.put()
        except Exception as ex:
            return False

    def increase_count(self, anagram_data, word):
        try:
...
class GenerateAnagram(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        template = JINJA_ENVIRONMENT.get_template('templates/generateAnagram.html')
        self.response.write(template.render(template_values))
...
class SearchAnagramWord(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        if self.request.GET.has_key('search'):
            ...
            template = JINJA_ENVIRONMENT.get_template('templates/searchWords.html')
            self.response.write(template.render(template_values))
...
class ShowWAllAnagramWords(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        template = JINJA_ENVIRONMENT.get_template('templates/displayWords.html')
        self.response.write(template.render(template_values))
